...
<template>
    ...
    <AppLayout>
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-foreground">Filiales</h1>
        ...
        <div class="bg-background rounded-xl border border-border overflow-hidden">
            <table class="w-full text-sm">
                <thead class="bg-accent/20 border-b border-border">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Nom</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Entité</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Pays</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Documents</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase">Statut</th>
                        <th class="px-4 py-3"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="filiale in filiales" :key="filiale.id" class="hover:bg-accent/20">
                        <td class="px-4 py-3 font-medium text-foreground">{{ filiale.name }}</td>
                        <td class="px-4 py-3 text-muted-foreground">{{ filiale.entity?.name }}</td>
                        <td class="px-4 py-3 text-muted-foreground">{{ filiale.country || '—' }}</td>
                        <td class="px-4 py-3 text-muted-foreground">{{ filiale.documents_count }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                                :class="filiale.is_active ? 'bg-emerald-500/15 text-green-700' : 'bg-red-100 text-red-600'">
                                {{ filiale.is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2 justify-end">
                                <button @click="openEdit(filiale)" class="text-primary hover:text-primary text-xs font-medium">Modifier</button>
                                <button @click="deleteFiliale(filiale)" class="text-destructive hover:text-red-700 text-xs font-medium">Supprimer</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-60" @click.self="showModal = false">
            <div class="bg-background rounded-2xl shadow-2xl max-w-sm w-full p-6">
                <h3 class="font-bold text-foreground mb-4">{{ editing ? 'Modifier la filiale' : 'Nouvelle filiale' }}</h3>
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1">Pays</label>
