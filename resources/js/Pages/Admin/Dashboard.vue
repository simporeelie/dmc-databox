    ...
    <AppLayout>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <div class="text-3xl font-bold text-gray-800">{{ stats.documents.toLocaleString() }}</div>
                <div class="text-xs text-gray-400 mt-1">{{ formatSize(stats.total_size) }} utilisés</div>
            </div>

            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-sm font-medium text-gray-500">Téléchargements</span>
                ...
                <div class="text-3xl font-bold text-gray-800">{{ stats.downloads.toLocaleString() }}</div>
                <div class="text-xs text-gray-400 mt-1">Total cumulé</div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <div class="text-3xl font-bold text-gray-800">{{ stats.active_users }}</div>
                <div class="text-xs text-gray-400 mt-1">{{ stats.users }} au total</div>
            </div>

            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <div class="flex items-center justify-between mb-3">
                    <span class="text-sm font-medium text-gray-500">Entités / Filiales</span>
                ...
                <div class="text-3xl font-bold text-gray-800">{{ stats.entities }}</div>
                <div class="text-xs text-gray-400 mt-1">{{ stats.filiales }} filiales</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <!-- Documents par type -->
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-800 mb-4">Répartition par type</h2>
                <div class="space-y-3">
                    <div v-for="(count, type) in by_type" :key="type" class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                            :class="typeColors[type] || 'bg-gray-100 text-gray-600'">
                        ...
                        <div class="flex-1">
                            <div class="flex justify-between text-sm mb-1">
                                <span class="font-medium text-gray-700">{{ typeLabels[type] || type }}</span>
                    ...
                    <div v-if="!Object.keys(by_type).length" class="text-sm text-gray-400 text-center py-4">
                        Aucun document
                    </div>
                </div>
            </div>
            ...
            <!-- Documents par année -->
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-800 mb-4">Documents par année</h2>
                <div class="space-y-3">
                    <div v-for="row in by_year" :key="row.year" class="flex items-center gap-3">
                        <span class="text-sm font-medium text-gray-500 w-12 flex-shrink-0">{{ row.year }}</span>
                        <div class="flex-1">
                        ...
                        <span class="text-sm font-semibold text-gray-700 w-8 text-right">{{ row.count }}</span>
                    </div>
                    <div v-if="!by_year?.length" class="text-sm text-gray-400 text-center py-4">
                        Aucun document
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-800 mb-4">Top entités</h2>
                <div class="space-y-3">
                    <div v-for="(row, i) in by_entity" :key="i" class="flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between text-sm mb-1">
                                <span class="font-medium text-gray-700 truncate">{{ row.name }}</span>
                                <span class="text-gray-400 ml-2 flex-shrink-0">{{ row.count }}</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-1.5">
                    ...
                    <div v-if="!by_entity?.length" class="text-sm text-gray-400 text-center py-4">
                        Aucun document
                    </div>
                </div>
            </div>
            ...
            <!-- Top téléchargements -->
            <div class="bg-white rounded-xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-800 mb-4">Top téléchargements</h2>
                <div class="space-y-3">
                    <div v-for="(doc, i) in top_downloaded" :key="doc.id" class="flex items-center gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400">{{ doc.entity?.name }} · {{ doc.file_extension?.toUpperCase() }}</p>
                        </div>
                        <span class="text-sm font-bold text-gray-700 flex-shrink-0">{{ doc.download_count }}</span>
                    </div>
                    <div v-if="!top_downloaded?.length" class="text-sm text-gray-400 text-center py-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <h2 class="font-semibold text-gray-800">Ajouts récents</h2>
            ...
            <div class="divide-y divide-gray-50">
                <div v-for="doc in recent_documents" :key="doc.id"
                    ...
                    <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0"
                        :class="{
                            ...
                            'bg-gray-100': doc.file_type === 'other',
                        }">
                        <svg class="w-4 h-4"
                            :class="{
                                ...
                                'text-gray-500': doc.file_type === 'other',
                            }"
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-400">{{ doc.entity?.name }} · {{ doc.category?.name }}</p>
                    </div>
                    <div class="text-right flex-shrink-0">
                        <p class="text-xs text-gray-500">{{ doc.uploader?.name }}</p>
                        <p class="text-xs text-gray-400">{{ new Date(doc.created_at).toLocaleDateString('fr-FR') }}</p>
                    </div>
                    <div class="flex items-center gap-1 text-xs text-gray-400 flex-shrink-0 w-14 justify-end">
                ...
                <div v-if="!recent_documents?.length" class="px-5 py-8 text-center text-sm text-gray-400">
